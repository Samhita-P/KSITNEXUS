...
class FacultyFeedback(models.Model):
    """Anonymous faculty feedback system"""
    ...
    # Anonymous submission - no direct student reference
...
class FacultyFeedbackSummary(models.Model):
    ...
    def update_summary(self):
        ...
        if feedbacks.exists():
            self.avg_teaching_quality = sum(f.teaching_quality for f in feedbacks) / feedbacks.count()
            self.avg_communication = sum(f.communication for f in feedbacks) / feedbacks.count()
            self.avg_punctuality = sum(f.punctuality for f in feedbacks) / feedbacks.count()
            self.avg_subject_knowledge = sum(f.subject_knowledge for f in feedbacks) / feedbacks.count()
            self.avg_helpfulness = sum(f.helpfulness for f in feedbacks) / feedbacks.count()
            self.avg_overall_rating = sum(f.overall_rating for f in feedbacks) / feedbacks.count()
