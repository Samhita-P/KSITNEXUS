"""
Management command to force add questions to each category
"""
...
class Command(BaseCommand):
    help = 'Force add questions to each chatbot category'

    def handle(self, *args, **options):
        self.stdout.write('Force adding questions to each category...')

        # Clear existing questions first
        ChatbotQuestion.objects.all().delete()
        ...
        for category_name, questions in category_questions.items():
            try:
                ...
                self.stdout.write(f'Added {added_count} questions to {category_name}')
