...
class OTPVerificationScreen extends ConsumerStatefulWidget {
  ...
  final String? userType; // For routing after registration
...
class _OTPVerificationScreenState extends ConsumerState<OTPVerificationScreen> {
  ...
  Future<void> _verifyOTP() async {
    ...
    if (_isLoading) {
      return; // Prevent multiple simultaneous verification attempts
    }
    ...
    try {
      if (widget.purpose == 'registration') {
        // Use the provider to update auth state properly
        final response = await ref.read(authStateProvider.notifier).verifyRegistrationOTP(
          widget.userId, 
          _otpController.text,
        );
        ...
        if (mounted) {
          // Get the updated user from auth state
          final authState = ref.read(authStateProvider);
          final user = authState.user;
          
          // Check if student needs to enter USN
          final requiresUsnEntry = response['requires_usn_entry'] as bool? ?? false;
          final userType = user?.userType ?? widget.userType;
          
          print('OTP verified successfully. User type: $userType, requires_usn_entry: $requiresUsnEntry');
          
          if (userType == 'faculty') {
            print('Navigating to faculty dashboard');
            context.go('/faculty-dashboard');
          } else if (requiresUsnEntry) {
            // Student needs to enter USN
            print('Navigating to USN entry screen');
            context.go('/usn-entry?userId=${widget.userId}');
          } else {
            // Student already has USN, go to dashboard
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  TextFormField(
                    ...
                    maxLength: 6,
                    onChanged: (value) {
                      // Removed auto-verification to prevent infinite loops
